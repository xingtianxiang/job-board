...
- **高亮 = 正在做的东西,自动派生**,两种来源:
  - **按 git 改动(零维护,推荐)**:给模块写好 `paths`(代码路径 glob,如 `src/scene/**`)。任何人 `sync` 时,网站看他当前分支改了哪些文件,命中就把对应模块高亮成他的色(✎ 在改)。连标 `doing` 都不用。
  - **按 `doing` 标记**:把某 feature 标 `status: doing` + `owner`,对应模块高亮(▶ 进行中)。适合“还没动代码、但下一步要做”。
  - git 改动优先级更高;14 天没动会自动淡出。
